...
import { confirmFrontCard } from '@/api/forgot-password'
...
export default function EkycCameraScreen() {
  ...
  const ekycFrontMutation = useMutation({
    mutationFn: (data: CameraCapturedPicture) =>
      confirmFrontCard(credentials, data),
    onSuccess: (data) => {
      ...
      router.push('/authentication/forget-password/back-card-confirmation')
    },
    onError: (error: Error) => {

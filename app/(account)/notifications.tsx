...
export default function Notifications() {
  ...
  const renderItem = ({ item }: { item: (typeof featuresData)[0] }) => (
    <View className="flex-row justify-start items-center py-3 w-full border-b border-gray-300">
      <View className="bg-[#3074E3] rounded-full w-2 h-2"></View>
      <View className="w-12 h-12 rounded-full border border-gray-400 border-opacity-40 ml-2">
        <Image source={item.icon} className="w-full h-full rounded-full" />
      </View>
      <View className="ml-2 flex-1">
        <Text className="text-secondary w-full">{item.description}</Text>
        <View className="flex-row justify-start items-center mt-2">
          <Text className=" text-tertiary text-sm">{item.time}</Text>
          <View className="bg-tertiary rounded-full w-1 h-1 left-5"></View>
